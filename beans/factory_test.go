...
func TestBeanFactoryGetBean(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			err:      "No candidate found for field 'notImplementInterface' with type beans.testNotImplementInterface",
		},
	}
...
func TestAutowireMultipleCandidatesWithMultiplePrimary(t *testing.T) {
	...
	g.Expect(err).Should(HaveOccurred())
	g.Expect(err.Error()).To(Equal("'2' primary candidates found for field 'test' with type beans.testInterface"))
}
...
func TestAutowireMultipleCandidatesWithNotImplementInterfaceLazyLoadBean(t *testing.T) {
	...
	_, err := bf.GetBean("test-bean")
	g.Expect(err).Should(HaveOccurred())
	g.Expect(err.Error()).To(Equal("'2' candidates found for field 'test' with type beans.testInterface"))
}
